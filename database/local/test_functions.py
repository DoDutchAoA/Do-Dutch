...
#        self.assertEqual(groupId, -1)
    ...
    def test_search_friend(self):
        friendIdList = f.searchUserByUsername("testAddFriend")
        self.assertEqual(len(friendIdList), 0)

        f.signUp("testAddFriend", "12345")
        friendIdList = f.searchUserByUsername("testAddFriend")
        self.assertEqual(len(friendIdList), 1)
    ...
    def test_remove_friend(self):
        ...
        result = f.removeFriend(ownerId, friendId)
        self.assertEqual(result["status"], True)
...
if __name__ == '__main__':
    with open('../../tests/test-reports/unittest/func_results.xml', 'w') as output:
        unittest.main(
